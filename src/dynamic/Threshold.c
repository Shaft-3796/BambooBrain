 ...
 * @return the threshold in a float array
 */
float *get_subproblem_threshold_all_values(const Config *config, const Subproblem *sp, const int feature_id) {
    if(!sp->instance_count) {bbprintf("Error: get_subproblem_threshold_all_values: subproblem has no instances\n"); exit(1);}
